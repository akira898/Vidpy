...
def visualGestore():
    ...
    with open ("projects\\"+projectToRun+"\\Vidpy"+"\\Images"+"\\Images And whenToputThem.txt", "r", encoding="utf-8") as iawtt:
        for line in iawtt:
            ...
            if os.path.exists("projects\\"+projectToRun+"\\Vidpy"+"\\Images"+"\\backgrounds.txt"):
                        with open("projects\\"+projectToRun+"\\Vidpy"+"\\Images"+"\\backgrounds.txt","r",encoding="utf-8") as backgrounds:
                            for line in backgrounds:
                                words=line.split()
                                print("untill the night falls")
                                backgroundSchudele[words[0]]={}
                                backgroundSchudele[words[0]]["start"]=int(words[1])

                                if os.path.exists("projects\\"+projectToRun+"\\Vidpy"+"\\Images"+"\\limit" + words[0] + ".txt"):
                                    with open("projects\\"+projectToRun+"\\Vidpy"+"\\Images"+"\\limit" + words[0] +".txt","r",encoding="utf-8") as readingMaximun:
                                        maxi=int(readingMaximun.read())
                                        backgroundSchudele[words[0]]["end"]=maxi
                                        print("please fuckme")
                                        img=words[1]
                                else:
                                    print("heyy")
